...
class cl_db_docparag { 
   ...
   function incluir ($db04_docum,$db04_idparag){ 
     ...
     $result = @pg_query("insert into db_docparag(
                                       db04_docum 
                                      ,db04_idparag 
                                      ,db04_ordem 
                       )
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = pg_query("insert into db_acountkey values($acount,3595,'$this->db04_docum','I')");
       $resac = pg_query("insert into db_acountkey values($acount,3596,'$this->db04_idparag','I')");
       $resac = pg_query("insert into db_acount values($acount,519,3595,'','".pg_result($resaco,0,'db04_docum')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = pg_query("insert into db_acount values($acount,519,3596,'','".pg_result($resaco,0,'db04_idparag')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = pg_query("insert into db_acount values($acount,519,3597,'','".pg_result($resaco,0,'db04_ordem')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($db04_docum=null,$db04_idparag=null) { 
     ...
     $sql .= " where  db04_docum = $this->db04_docum
 and  db04_idparag = $this->db04_idparag
";
     $resaco = $this->sql_record($this->sql_query_file($this->db04_docum,$this->db04_idparag));
     if($this->numrows>0){       $resac = pg_query("select nextval('db_acount_id_acount_seq') as acount");
       $acount = pg_result($resac,0,0);
       $resac = pg_query("insert into db_acountkey values($acount,3595,'$this->db04_docum','A')");
       $resac = pg_query("insert into db_acountkey values($acount,3596,'$this->db04_idparag','A')");
       if(isset($GLOBALS["HTTP_POST_VARS"]["db04_docum"]))
         $resac = pg_query("insert into db_acount values($acount,519,3595,'".pg_result($resaco,0,'db04_docum')."','$this->db04_docum',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["db04_idparag"]))
         $resac = pg_query("insert into db_acount values($acount,519,3596,'".pg_result($resaco,0,'db04_idparag')."','$this->db04_idparag',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["db04_ordem"]))
         $resac = pg_query("insert into db_acount values($acount,519,3597,'".pg_result($resaco,0,'db04_ordem')."','$this->db04_ordem',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     $result = @pg_exec($sql);
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
   ...
   function excluir ($db04_docum=null,$db04_idparag=null) { 
     $this->atualizacampos(true);
     $resaco = $this->sql_record($this->sql_query_file($this->db04_docum,$this->db04_idparag));
     if(($resaco!=false)||($this->numrows!=0)){
       $resac = pg_query("select nextval('db_acount_id_acount_seq') as acount");
       $acount = pg_result($resac,0,0);
       $resac = pg_query("insert into db_acountkey values($acount,3595,'".pg_result($resaco,$iresaco,'db04_docum')."','E')");
       $resac = pg_query("insert into db_acountkey values($acount,3596,'".pg_result($resaco,$iresaco,'db04_idparag')."','E')");
       $resac = pg_query("insert into db_acount values($acount,519,3595,'','".pg_result($resaco,0,'db04_docum')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = pg_query("insert into db_acount values($acount,519,3596,'','".pg_result($resaco,0,'db04_idparag')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = pg_query("insert into db_acount values($acount,519,3597,'','".pg_result($resaco,0,'db04_ordem')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     ...
     $sql2 = "";
      ...
      $sql2 .= " db04_docum = $this->db04_docum ";
}
      if($this->db04_idparag != ""){
      if($sql2!=""){
        $sql2 .= " and ";
      }
      $sql2 .= " db04_idparag = $this->db04_idparag ";
}
