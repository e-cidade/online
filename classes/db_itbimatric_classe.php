...
class cl_itbimatric { 
   ...
   var $campos = "
                 it06_guia = int8 = Número da guia de ITBI 
   ...
   function incluir ($it06_guia,$it06_matric){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = pg_query("insert into db_acountkey values($acount,5410,'$this->it06_guia','I')");
       $resac = pg_query("insert into db_acountkey values($acount,5409,'$this->it06_matric','I')");
       $resac = pg_query("insert into db_acount values($acount,797,5410,'','".AddSlashes(pg_result($resaco,0,'it06_guia'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = pg_query("insert into db_acount values($acount,797,5409,'','".AddSlashes(pg_result($resaco,0,'it06_matric'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($it06_guia=null,$it06_matric=null) { 
     ...
     $sql .= " where  it06_guia = $this->it06_guia
 and  it06_matric = $this->it06_matric
";
     $resaco = $this->sql_record($this->sql_query_file($this->it06_guia,$this->it06_matric));
     if($this->numrows>0){       $resac = pg_query("select nextval('db_acount_id_acount_seq') as acount");
       $acount = pg_result($resac,0,0);
       $resac = pg_query("insert into db_acountkey values($acount,5410,'$this->it06_guia','A')");
       $resac = pg_query("insert into db_acountkey values($acount,5409,'$this->it06_matric','A')");
       if(isset($GLOBALS["HTTP_POST_VARS"]["it06_guia"]))
         $resac = pg_query("insert into db_acount values($acount,797,5410,'".AddSlashes(pg_result($resaco,0,'it06_guia'))."','$this->it06_guia',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       if(isset($GLOBALS["HTTP_POST_VARS"]["it06_matric"]))
         $resac = pg_query("insert into db_acount values($acount,797,5409,'".AddSlashes(pg_result($resaco,0,'it06_matric'))."','$this->it06_matric',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     $result = @pg_exec($sql);
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
   ...
   function excluir ($it06_guia=null,$it06_matric=null) { 
     $resaco = $this->sql_record($this->sql_query_file($it06_guia,$it06_matric));
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         $resac = pg_query("select nextval('db_acount_id_acount_seq') as acount");
         $acount = pg_result($resac,0,0);
         $resac = pg_query("insert into db_acountkey values($acount,5410,'".pg_result($resaco,$iresaco,'it06_guia')."','E')");
         $resac = pg_query("insert into db_acountkey values($acount,5409,'".pg_result($resaco,$iresaco,'it06_matric')."','E')");
         $resac = pg_query("insert into db_acount values($acount,797,5410,'','".AddSlashes(pg_result($resaco,$iresaco,'it06_guia'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = pg_query("insert into db_acount values($acount,797,5409,'','".AddSlashes(pg_result($resaco,$iresaco,'it06_matric'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
