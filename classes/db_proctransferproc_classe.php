...
class cl_proctransferproc { 
   ...
   var $campos = "
                 p63_codtran = int8 = Transferencia 
   ...
   function excluir ($p63_codtran=null,$p63_codproc=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         $resac = db_query("select nextval('db_acount_id_acount_seq') as acount");
         $acount = pg_result($resac,0,0);
         $resac = db_query("insert into db_acountkey values($acount,2480,'$this->p63_codtran','E')");
         $resac = db_query("insert into db_acountkey values($acount,2481,'$this->p63_codproc','E')");
