...
if(!isset($emite_recibo_protocolo)){
 ...
 // $k00_descr = $k00_descr." - ".db_getsession("DB_anousu");
  ...
  if(!isset($numpre_unica) || $numpre_unica ==""){
    ...
    for($i = 1;$i < sizeof($numpres);$i++) {
      ...
      pg_query($sql) or die("Erro(26) inserindo em db_reciboweb: ".pg_errormessage());
      //echo $sql . "<br>";
    }
  }else{
 ...
 // die ($sql);
  ...
  $Recibo = pg_query($sql) or die("$sql");
pg_query("COMMIT");	
...
if(!empty($HTTP_POST_VARS["ver_matric"])) {
  ...
  $Identificacao = pg_query("select z01_nome,z01_cgccpf,z01_ender,z01_munic,z01_uf,z01_cep,nomepri,j39_compl,j39_numero,j13_descr,j34_setor||'.'||j34_quadra||'.'||j34_lote as sql
                            from proprietario
...
if(isset($tipo)){
  ...
  $resulttipo = pg_query("select k03_tipo from arretipo where k00_tipo = $tipo");
