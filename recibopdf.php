 ...
 */
  ...
  if(!isset($dados_recibo)) {// ################### SE FOR EMITE RECIBO  #######################################
   // die(" emite ");
    ...
    $result1= pg_query($sql1) or die ($sql1);
    if (pg_num_rows($result1) > 0) {
      
      db_fieldsmemory($result1,0);
      if($q24_inscr != 0) {
        $result= pg_query("insert into arreinscr (k00_numpre,k00_inscr) values(".$q20_numpre.",".$q24_inscr.")");
      }
    }
...
/*
echo"issplan ";
db_criatabela(pg_exec("select * from issplan where q20_planilha = $planilha"));
echo"arrecad ";
db_criatabela(pg_exec("select * from arrecad where k00_numpre =$q20_numpre"));
echo"issvar ";
db_criatabela(pg_exec("select * from issvar where q05_numpre =$q20_numpre"));
echo"arreinscr";
db_criatabela(pg_exec("select * from arreinscr where k00_numpre =$q20_numpre"));
