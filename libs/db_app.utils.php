...
class db_app {
  ...
  */  
  public function import($sClasse) {
    
    $aBasePath = array("model/", "libs/");
    if (preg_match('/(\.\*)$/', $sClasse)) {
      ...
      foreach ($aBasePath as $sBasePath) {
        ...
        $sImportFile = str_replace(".", "/", $sClasse) .$sSufix;
        if (is_file($sBasePath . $sImportFile)) { 
          require_once($sBasePath . $sImportFile);
        }
      }
    }
  }
