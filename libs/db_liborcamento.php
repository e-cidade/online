...
class cl_permissaodotacao {
  ...
  function retornarecord() {
    ...
    $this->result = pg_exec($this->dotacoes);
...
function db_receitappa($anoini, $db_where = false, $retsql = false) {
  ...
  function db_dotacaosaldo($nivel = 8, $tipo_nivel = 1, $tipo_saldo = 2, $descr = true, $where = '', $anousu = null, $dataini = null, $datafim = null, $primeiro_fim = 8, $segundo_inicio = 0, $retsql = false, $tipo_balanco = 1, $desmembra_segundo_inicio = true, $subelemento = 'nao') {
    ...
    select *,
    (case when o58_projativ < 2000 and o58_projativ > 999     then $tipo_pa else 0 end) as proj,
    (case when o58_projativ > 1999 and  o58_projativ < 3000 then $tipo_pa else 0 end) as ativ,
    (case when o58_projativ < 1000 or o58_projativ > 2999 then $tipo_pa else 0 end) as oper,
    (case when o58_codigo =  1 then $tipo_pa else 0 end) as ordinario,
    (case when o58_codigo <> 1 then $tipo_pa else 0 end) as vinculado
    from 
    (select o58_anousu, 
    o58_orgao,
    o58_unidade,
    ...
    */
    ...
    select distinct orcdotacao.o58_coddot 
    from orcdotacao,
    db_usupermemp 
    inner join db_permemp on db21_codperm = db20_codperm   
    where o58_anousu = ".db_getsession("DB_anousu")." and 
    db21_id_usuario = ".db_getsession("DB_id_usuario")." and 
    db20_anousu = ".db_getsession("DB_anousu")." and 
    ( ( o58_orgao = db20_orgao ) or 
    ( o58_projativ = db20_projativ ) or 
    (o58_unidade = db20_unidade) or 
    (o58_funcao = db20_funcao) or 
    (o58_subfuncao = db20_subfuncao) or 
    (o58_programa = db20_programa) or 
    (o58_codele = db20_codele) or 
    (o58_codigo = db20_codigo) 
    )
    )*/ 
    order by
    o58_orgao,
    ...
    //echo "<br><br>".$sql;
    $result1 = pg_exec($sql);
    // $result1 = pg_exec("select * from work_dotacao");
    ...
    if ($descr == true) {
      if ($retsql == false) {
        $resultdotacao = pg_exec($sql2);
      } else {
        $resultdotacao = $sql2;
      }
    } else {
      if ($retsql == false) {
        $resultdotacao = pg_exec($sql);
      } else {
        $resultdotacao = $sql;
      }
    }
  ...
  if( $anousu > 2007 ){
    ...
    $result = pg_exec($sql);
    //$result = pg_exec("select * from work_receita");
    //db_criatabela($result);exit;
...
//db_criatabela($result);exit;
 ...
 //   $result = pg_exec($sql);
  //  db_criatabela($result);exit;
    ...
    $result = pg_exec($sql);
    //db_criatabela($result); exit;
    //$result = pg_exec("select * from work_receita");db_criatabela($result);exit;
  ...
  // aqui termina o codigo de 2007
    ...
    if ($descr == true)
      if ($query == false)
         $resultreceita = pg_exec($sql2);
      else
         $resultreceita = $sql2;
    else
      if ($query == false)
         $resultreceita = pg_exec($sql);
      else
        $resultreceita = $sql;
    //db_criatabela($resultreceita);
...
*/        
    ...
    function getParametros($nomes = false) {
      ...
      if ($nomes == true) {
        ...
        $sql = "select o41_descr from orcunidade 
        inner join orcdotacao on o58_unidade = o41_unidade and o58_orgao = o41_orgao and o58_anousu=o41_anousu
        where o41_anousu=".db_getsession("DB_anousu")." and ".$this->unidade;
        $rr = @ pg_query($sql);
        if ($rr != false) {
          $sp = "";
          for ($x = 0; $x < @ pg_numrows($rr); $x ++) {
