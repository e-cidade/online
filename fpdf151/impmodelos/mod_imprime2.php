...
for ($i = 0;$i < 2;$i++){
  ...
  $this->objpdf->cell(25,9,db_formatar($this->totalrec,"f"),0,0,"R");
  $this->objpdf->setxy(181,$xlin+81.5);
  $this->objpdf->cell(25,9,db_formatar($this->totaldesc,"f"),0,0,"R");
  $this->objpdf->setxy(181,$xlin+92);
  $this->objpdf->cell(25,9,db_formatar($this->totalacres,"f"),0,0,"R");
...
if ($this->loteamento == true) {
  ...
  x.k99_desconto,
  x.k00_ano,
  fc_calcula(x.k99_numpre,x.k99_numpar,x.k00_receit,'" . date("Y-m-d",db_getsession("DB_datausu")) . "', '" . date("Y-m-d",db_getsession("DB_datausu")) . "', " . db_getsession("DB_anousu") . ") from
  (
  select 
  distinct
  k99_numpre,
