...
$pdf->MultiCell(0,6,'Comprovante de Solicitação para impressão de documentos fiscais- AIDOF ',0,"C",0);
$pdf->SetFont($Letra,'B',12);
$pdf->Ln(3);
$pdf->MultiCell(0,6,'EMPRESA SOLICITANTE :',0,"J",0);
$pdf->Ln(3);
$pdf->SetFont($Letra,'',10);
$result = pg_exec("select * from cgm inner join issbase on q02_numcgm = z01_numcgm where q02_inscr = $inscricao");
...
$pdf->Ln(3);
$pdf->SetFont($Letra,'',10);
$result = pg_exec("select * from aidof where y08_codigo = $codigo");
if(pg_numrows($result) > 0){
  db_fieldsmemory($result,0);
}
$pdf->MultiCell(0,6,'codigo : '.@$codigo ,0,"J",0);
$pdf->MultiCell(0,6,'Tipo de Nota : '.@$nota,0,"J",0);
$pdf->MultiCell(0,6,'Quantidade de Notas solicitadas : '.(@$y08_quantsol),0,"J",0);
