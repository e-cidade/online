...
error_reporting('** Informe Seus Clientes **');
session_start();
...
$rsVerificaParametros  = pg_query($sqlVerificaParametros);
...
$rsVerificaListaAnterior = pg_query($sqlVerificaListaAnterior);
...
$sqlCriaLista = " select * 
                   ...
                   where p11_numcgm   = {$numCgm}
                     and p11_fechado  = 'f' ";
//die($sqlCriaLista);
$rsCriaLista = pg_query($sqlCriaLista);
...
if ($iSql > 0) {
    ...
    //die($sqlListaInscr);
		$rsListaInscr = pg_query($sqlListaInscr);
 ...
 <tr height="50">
	...
	$sqlListaInscrCb = " select * 
	                       ...
	                       where p11_numcgm     = {$cgmlogin}
	                         and p11_processado = 'f' ";
	//die($sqlListaInscrCb);
	$rsListaInscrCb = pg_query($sqlListaInscrCb);
  ...
  if (isset($iRegra)) {
     ...
     $iListainscr  = pg_numrows($rsListainscr);
	     ...
	     if ($iRegra == 4 || $iRegra == 5) {   
			     ...
			     $rsVerificaBaixa = pg_query($sqlVerificaBaixa);
